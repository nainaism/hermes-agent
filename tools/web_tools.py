...
def _get_backend() -> str:
    ...
    if configured in {"parallel", "firecrawl", "tavily", "exa", "searxng", "brave-free", "ddgs", "tinyfish", "xai"}:
        return configured
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        # tinyfish, xai) now live as plugins; the dispatcher is just a registry
        # lookup + delegation. Sync only — every provider's search() is sync.
...
async def web_extract_tool(
    ...
    try:
        ...
        if not safe_urls:
            ...
            # All providers (brave-free, ddgs, searxng, exa, parallel,
            # tavily, firecrawl, tinyfish, xai) now live as plugins. The
            # dispatcher is a registry lookup + delegation. Some providers'
            # extract() is async (parallel, firecrawl), others sync (exa,
            # tavily, tinyfish, xai) — we detect coroutine functions and await;
            # sync functions run inline (the policy gate, SSRF re-check,
            # etc. live inside the provider itself for the firecrawl
            # per-URL loop).
            _ensure_web_plugins_loaded()
            from agent.web_search_registry import (
                    ...
                    return json.dumps(
                        {
                            ...
                            "error": (
                                ...
                                "tavily, exa, parallel, or tinyfish."
                            ),
                        },
                        ensure_ascii=False,
                    )
            ...
            # extract(); exa + tavily + tinyfish are sync.
            import inspect
...
def check_web_api_key() -> bool:
    ...
    if configured in {"exa", "parallel", "firecrawl", "tavily", "searxng", "brave-free", "ddgs", "tinyfish", "xai"}:
        return _is_backend_available(configured)
    return any(
        _is_backend_available(backend)
        for backend in ("exa", "parallel", "firecrawl", "tavily", "searxng", "brave-free", "ddgs", "tinyfish", "xai")
    )
